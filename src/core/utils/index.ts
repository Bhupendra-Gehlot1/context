...
const QUESTION_STARTERS = [
  'can ', 'could ', 'should ', 'would ', 'anyone ', 'does ', 'will ',
  'is ', 'are ', 'has ', 'have ', 'do ', 'when ', 'where ', 'why ', 'how ', 'what ',
];
...
const STOP_WORDS = new Set([
  ...
  'just', 'not', 'also', 'so', 'if', 'about', 'up', 'as', 'into',
  'then', 'than', 'too', 'very', 'any', 'all', 'some', 'more', 'no',
]);
...
const TOPIC_SEEDS: Record<string, string[]> = {
  Deployment: ['deploy', 'deployment', 'release', 'ship', 'production', 'staging', 'pipeline', 'ci', 'cd'],
  'Bug Fixes': ['bug', 'fix', 'error', 'issue', 'broken', 'crash', 'exception', 'null', 'undefined'],
  'Code Review': ['review', 'pr', 'pull request', 'merge', 'branch', 'commit', 'diff'],
  API: ['api', 'endpoint', 'request', 'response', 'rest', 'graphql', 'fetch', 'axios'],
  Frontend: ['frontend', 'ui', 'component', 'react', 'css', 'tailwind', 'style', 'design'],
  Backend: ['backend', 'server', 'database', 'db', 'query', 'migration', 'schema'],
  Testing: ['test', 'tests', 'testing', 'spec', 'jest', 'vitest', 'coverage', 'unit', 'e2e'],
  Performance: ['performance', 'slow', 'fast', 'optimize', 'cache', 'latency', 'load'],
  Meeting: ['meeting', 'standup', 'sync', 'call', 'demo', 'presentation', 'discuss'],
};
...
export function buildSummary(messages: Message[]): ConversationSummary {
  if (messages.length === 0) {
    return { participants: [], topics: [], recentActivity: [], openQuestions: [], messageCount: 0 };
  }
...
export function groupMessages(messages: Message[]): MessageGroup[] {
  ...
  messages.forEach((msg) => {
    ...
    if (
      ...
      groups.push({ user_name: msg.user_name, messages: [msg], firstTimestamp: msg.created_at });
    }
  });
...
const AVATAR_COLORS = [
  '#6c63ff', '#ec4899', '#f59e0b', '#10b981', '#3b82f6',
...
/** One entry per presence key — avoids counting duplicate refs (tabs, heartbeats). */
export function parsePresenceUsers(state: Record<string, unknown[]>): PresenceUser[] {
