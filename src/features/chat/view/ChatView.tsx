...
export default function ChatView() {
  ...
  useUserSync();
  const { messages, sendMessage, isSending, isLoading, error, isConnected, retryLoad } =
    useChatController();
  ...
  const header = (
    <header className="flex shrink-0 items-center justify-between border-b border-surface-border px-4 py-3 lg:px-6">
      <div>
        <h1 className="text-lg font-semibold text-slate-100">{UI_COPY.chat.title}</h1>
  ...
  const footer = (
    <footer className="shrink-0 border-t border-surface-border px-4 py-3 lg:px-6">
      {error && messages.length > 0 && (
        <p className="mb-2 text-xs text-red-400" role="alert">{error}</p>
      )}
      <TypingIndicator typingUsers={typingUsers} />
      <MessageInput sending={isSending} onSend={sendMessage} onTyping={() => setTyping(true)} />
    </footer>
  );
  ...
  return (
    <>
      <ChatLayout header={header} sidebar={sidebar} footer={footer} statusBanner={statusBanner}>
        {mainContent}
      </ChatLayout>
      <SummaryModal open={showSummary} summary={summary} onClose={() => setShowSummary(false)} />
    </>
  );
}
